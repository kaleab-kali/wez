...
const PlacementRow = React.memo(
	({ placement, canEndPlacement }: { readonly placement: Placement; readonly canEndPlacement: boolean }) => {
		...
		const canEnd = canEndPlacement && placement.status === "active";
					...
					<div className="flex flex-wrap items-start justify-between gap-3 border-b pb-3">
						<div className="min-w-0">
							...
							<p className="mt-1 text-xs text-muted-foreground">
								{placement.role?.name ?? placement.roleId} -{" "}
								{placement.station?.name ?? placement.stationId.slice(0, 8)}
						...
						<div className="mt-4 rounded-md border bg-muted/20 p-3">
							<div className="grid gap-3 md:grid-cols-[1fr_120px_120px]">
								...
								<div>
									<label
										className="text-xs font-medium text-muted-foreground"
										htmlFor={`rating-worker-${placement.id}`}
									>
...
const PlacementsPage = React.memo(() => {
	...
	const canFinalizePlacement = React.useMemo(
		() => hasAnyStaffRole(userRoles, STAFF_ACCESS_ROLES.placementFinalization),
		[userRoles],
	);
	const canEndPlacement = React.useMemo(
		() => hasAnyStaffRole(userRoles, STAFF_ACCESS_ROLES.placementEnding),
		[userRoles],
	);
	...
	return (
		<div className="max-w-6xl space-y-4">
			...
			{canFinalizePlacement && <FreshPlacementForm />}

			{isLoading && <p className="text-sm text-muted-foreground">{t("common.loading")}</p>}
			<div className="space-y-3">
				{data?.data.map((placement) => (
					<PlacementRow key={placement.id} placement={placement} canEndPlacement={canEndPlacement} />
				))}
