...
import { effectiveStaffRoles, manageableStaffRoles } from "#shared/lib/staff-roles";
...
const StaffCreateForm = React.memo(({ manageableRoles }: { readonly manageableRoles: readonly StaffRole[] }) => {
	const { t } = useTranslation();
	const create = useCreateStaffUser();
	const [name, setName] = React.useState("");
	const [email, setEmail] = React.useState("");
	const [primaryRole, setPrimaryRole] = React.useState<StaffRole | "">("");
	const [temporaryPassword, setTemporaryPassword] = React.useState("");
	const [generatedPassword, setGeneratedPassword] = React.useState("");
	const [error, setError] = React.useState("");

	React.useEffect(() => {
		if (primaryRole && manageableRoles.includes(primaryRole)) return;
		setPrimaryRole(manageableRoles[0] ?? "");
	}, [manageableRoles, primaryRole]);
	...
	const onSubmit = React.useCallback(
		async (e: React.FormEvent) => {
			...
			if (!primaryRole) {
				setError(t("staffUsers.noManageableRoles"));
				return;
			}
			try {
			...
			<CardContent>
				<form onSubmit={onSubmit} className="grid grid-cols-1 gap-3 md:grid-cols-4">
					...
					<div className="flex justify-end md:col-span-4">
						<Button type="submit" disabled={create.isPending || !name || !email || !primaryRole}>
...
const RoleAssignmentForm = React.memo(
	({ user, manageableRoles }: { readonly user: StaffUser; readonly manageableRoles: readonly StaffRole[] }) => {
		const { t } = useTranslation();
		...
		React.useEffect(() => {
			if (role && manageableRoles.includes(role)) return;
		...
		return (
			<form onSubmit={onSubmit} className="grid grid-cols-1 gap-2 md:grid-cols-4">
				...
				<Button type="submit" size="sm" disabled={assign.isPending || !role || (needsScope && !scopeId)}>
...
const StaffUserCard = React.memo(
	({ user, manageableRoles }: { readonly user: StaffUser; readonly manageableRoles: readonly StaffRole[] }) => {
		const { t } = useTranslation();
		...
		return (
			<Card>
				...
				<CardContent className="space-y-3">
					...
					<RoleAssignmentForm user={user} manageableRoles={manageableRoles} />
				</CardContent>
			</Card>
		);
	},
...
const StaffUsersPage = React.memo(() => {
	...
	const manageableRoles = React.useMemo(() => manageableStaffRoles(userRoles), [userRoles]);
	...
	return (
		<div className="max-w-5xl space-y-4">
			...
			<StaffCreateForm manageableRoles={manageableRoles} />
			<div className="space-y-3">
				{isLoading && <p className="text-sm text-muted-foreground">{t("common.loading")}</p>}
				{data?.map((user) => (
					<StaffUserCard key={user.id} user={user} manageableRoles={manageableRoles} />
				))}
				{data && data.length === 0 && (
