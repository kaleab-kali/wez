...
export class StaffUsersController {
	...
	async create(@Body() dto: CreateStaffUserDto, @Req() req: WezRequest & { adminUser?: { id: string } }) {
		return { data: await this.service.create(dto, req.adminUser?.id ?? "system") };
	}
	...
	async assignRole(
		...
		@Req() req: WezRequest & { adminUser?: { id: string } },
	) {
		return { data: await this.service.assignRole(id, dto, req.adminUser?.id ?? "system") };
	}
